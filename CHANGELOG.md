...
### Changed
- **BREAKING:** error fields renamed — `code` → `errorCode`, `correlationId` → `requestId`.
- Error parsing now decodes the real gateway envelope
  `{ statusCode, errorCode, message, timestamp, requestId }`; `requestId` is also
  read from the `X-Request-Id` response header.
- User-Agent standardized to `prompt-helm-sdk-android/<version>`.
